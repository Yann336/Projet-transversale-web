        ...
        <div class="grid">
            <article>
                <header>
                    <div class="article">
                        <img 
                            src="<?= htmlspecialchars($basket["PathPicture"], ENT_QUOTES, 'UTF-8') ?>" 
                            alt="Image">
                    </div>
                </header>
                <a href="index.php?page=lamp-details&id=<?= htmlspecialchars($basket['idLamp'], ENT_QUOTES, 'UTF-8') ?>">
                    Voir plus
                </a>
            </article>

            <form method="post">
                <input type="hidden" name="id" value="<?= htmlspecialchars($basket['idBasket'], ENT_QUOTES, 'UTF-8') ?>">
                <input
                    type="submit"
...
<?php } } ?>
    ...
    <div class="total-container">
        <span class="label">
            Total à payer <?= htmlspecialchars($price, ENT_QUOTES, 'UTF-8') ?> €
        </span>
    </div>

    <div>
        <a href="index.php?page=achat">
            <button type="button" class="btn-livraison">
...
<section id="Récapitulatif">
    ...
    <div class="total-commande">
        <span>
            Votre commande contient <?= htmlspecialchars($nbarticle, ENT_QUOTES, 'UTF-8') ?> articles
        </span>
    </div>
